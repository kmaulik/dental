...
class Messageboard extends CI_Controller {
	...
	public function index()
	{
		...
		$where=['to_id' => $this->session->userdata['client']['id'],'is_deleted' => '0'];
		$config['base_url'] = base_url().'messageboard/index';
		$config['total_rows'] = $this->Messageboard_model->get_message_count('msg_inbox',$where);
		$config['per_page'] = 10;
		$offset = $this->input->get('per_page');
		$config = array_merge($config,pagination_front_config());       
		$this->pagination->initialize($config);
		$data['messages']=$this->Messageboard_model->get_message_result('msg_inbox',$where,$config['per_page'],$offset);
		...
		$data['subview']="front/messageboard/message_inbox";
